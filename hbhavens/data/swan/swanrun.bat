...
if exist inputs\%inputfile%.swn goto OK2
  echo.
  echo Error: file inputs\%inputfile%.swn does not exist
  goto END
:OK2
...
copy inputs\%inputfile%.swn INPUT >> nul
...
if not %nprocs%==1 goto PARALLEL2
  if exist PRINT copy PRINT print_files\%inputfile%.prt >> nul
  if exist PRINT del PRINT
  goto OK5
:PARALLEL2
...
if exist Errfile copy Errfile error_files\%inputfile%.erf >> nul
if exist Errfile del Errfile
if exist ERRPTS copy ERRPTS error_files\%inputfile%.erp >> nul
