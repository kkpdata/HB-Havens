...
class IterationResults(ExtendedDataFrame):
    ...
    # normal properties
    _metadata = ["settings", "description", "initialized", "hydraulic_loads", "swanio", "iter_columns"]
    ...
    def initialize(self):
        ...
        for col, vals in self.hydraulic_loads.items():
            if col == "Description":
    ...
    def read_results(self, step, progress_function=None):
        ...
        arrs = {col: vals.loc[nonzero].values for col, vals in self.items()}
        ...
        for i, hydrodynamicdataid in enumerate(hydrodynamicdataids):
            ...
            if not os.path.exists(fpath):
                raise OSError(
                    'Resultaten voor case "{}" niet gevonden op de volgende locatie:\n{}'.format(
                        case_id, fpath.replace("/", "\\")
                    )
                )
        ...
        if step == "I1":
            ...
            arrs["Hs rand 3"][idx] = (
                (arrs["Hs"] - arrs["Hs steunpunt 1"])
                * ((arrs["Hs rand 2"] - arrs["Hs"]) / (arrs["Hs steunpunt 2"] - arrs["Hs steunpunt 1"]))
                + arrs["Hs"]
            )[idx]
            arrs["Tp,s rand 3"][idx] = (
                (arrs["Tp"] - arrs["Tp,s steunpunt 1"])
                * ((arrs["Tp,s rand 2"] - arrs["Tp"]) / (arrs["Tp,s steunpunt 2"] - arrs["Tp,s steunpunt 1"]))
                + arrs["Tp"]
            )[idx]
...
class CalculationResults(ExtendedDataFrame):
    ...
    def load_from_pickle(self, path):
        ...
        # for name, col in df[self.columns].items():
        #     self[name] = col
        # self.index = df.index
    ...
    def calculate_total_energy(self):
        ...
        if (
            self.settings["calculation_method"]["include_pharos"]
            or self.settings["calculation_method"]["include_hares"]
        ):
        ...
        # Without Pharos or Hares
            ...
            a = factor_E_W * self["Tmm10_W"] - factor_E_D * self["Tmm10_D"]
            b = factor_E_TR * self["Tmm10_TR"] - factor_E_W * self["Tmm10_W"]
            Tm_10_weighted = np.maximum(
                ((factor_E_D * self["Tmm10_D"] + a + b) / (factor_E_D + c + d)).round(3).fillna(0.0), 0.0
            )
            Tm_10_weighted[np.isinf(Tm_10_weighted)] = 0.0
            self["Tm-1,0 swan"] = Tm_10_weighted
    ...
    def read_results(self, step, progress_function=None):
        ...
        check_crds = np.vstack(
            [(row.geometry.x, row.geometry.y) for row in self.result_locations.sort_values(by="Naam").itertuples()]
        ).round(4)
        ...
        for hydraulicloadid, row in self.iteration_results.sort_index().iterrows():
            ...
            if len(crds) != len(check_crds):
                raise ImportError(
                    "The number of locations in the SWAN Spectrum differ from the number of HB Havens locations."
                )
            ...
            if (diff > 0.1).any():
                ...
                if any(u[c > 1]):
                    raise IndexError(
                        "The SWAN output locations do not match the HB Havens result locations. Cannot find unique pairs based on nearest neighbours."
                    )
                if any(distance > 1):
...
class Swan:
    ...
    def generate(self, step, progress_function=None):
        ...
        if step.startswith("I"):
            # support location
            self.swanio.writeSupportLocation(
                os.path.join(folder, "points"), self.mainmodel.schematisation.support_location
            )
        else:
    ...
    def create_input_files(self, step, folder, progress_function=None):
        ...
        for hydrodynamicdataid, hydraulic_load in self.iteration_results.iterrows():
            ...
            waterlevel = (
                hydraulic_load["Water level"] if not np.isnan(hydraulic_load["Water level"]) else hydraulic_load["h"]
            )
        ...
        with open(fpath, "w") as fp:
            ...
            fp.write(
                "\n".join(
                    (
                        "call swanrun.bat "
                        + self.iteration_results.loc[indices, "Load combination"]
                        .str.replace("=", "_")
                        .str.replace(" ", "_")
                    ).tolist()
                )
